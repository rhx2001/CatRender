...
void MaterialManager::loadTextureImage(std::string path)
{
	...
	materials[textureId] = std::make_shared< PBRMaterial>(materialId, BasicDynamicOffset * materialId, std::vector<std::shared_ptr<MaterialViewer>>{ materialViewers[ImageViewerID] }, "UnNamedMaterial" + std::to_string(materialId));
}
