...
public:
    ...
    std::unordered_map<uint32_t, std::shared_ptr<PBRMaterial>>& getMaterials() { return materials; }
	std::shared_ptr<PBRMaterial>& getMaterial(uint32_t materialID) { return materials[materialID]; }
...
private:
    ...
    std::unordered_map<uint32_t, std::shared_ptr<PBRMaterial>> materials;
