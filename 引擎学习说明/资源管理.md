...
- 使用符号（如PBR、PBR_trans等）管理不同类型的shader
- 负责创建ShaderModule和编译shader
- 具备良好的扩展性，可以添加mesh shader、光锥shader等
- 预留了shader热重载的功能接口

## PipelineManager

- 根据shader类型创建渲染管线
- 实现"一类材质对应一个pipeline"的模式，而不是每个材质一个pipeline
- 当遇到新的shader组合时动态创建新pipeline